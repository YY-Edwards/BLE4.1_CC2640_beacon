...
//Éè¶¨¶¯Ì¬¹ã²¥ºÍ¶¯Ì¬É¨Ãè»ØÓ¦µÄµÚÒ»´Î´¥·¢ÑÓ³Ù
...
//Ìí¼Ó¶¯Ì¬¹ã²¥Êý¾ÝºÍ¶¯Ì¬É¨Ãè»ØÓ¦Êý¾ÝÊÂ¼þ
...
//¶¨Òå¹ã²¥Êý¾Ý¸üÐÂºÍÉ¨Ãè»ØÓ¦Êý¾Ý¸üÐÂ¶¨Ê±Æ÷½á¹¹ÌåÊý¾Ý
...
static void update_scanrsp_data()
{
      //Ìî³ä×Ô¶¨ÒåÉ¨Ãè»ØÓ¦Êý¾Ý
    VOID memset(&scanRspData[2], 0x00, 8);//ÔÝ¶¨Éè±¸Ãû³Æ×î´óÖ§³Ö8¸öassic×Ö·û
...
//    scanRspData[13] = HI_UINT16((uint16)(SETTING_BEACON_ADV_INTERVAL/0.625));;
    //Éè¶¨·¢Éä¹¦ÂÊ
    //if(SETTING_BEACON_TX_POWER < LL_EXT_TX_POWER_0_DBM)//¸ºÊý
...
//  set_customerInfo(&last_customerStorageBeaconInfo);
  ...
  {//µÚÒ»´ÎÉÏµç£¬ÅäÖÃÎªÄ¬ÈÏÖµ
    ...
    //×¢²á´®¿Ú½ÓÊÕÊý¾Ý¼à²âÊÂ¼þ
    //µ¹ÊýµÚ¶þ¸ö²ÎÊý¾ö¶¨ÁË´´½¨£¬È»ºóµÈ´ý±»Æô¶¯-Util_startClock()
    //´Ëº¯Êý¿ÉÒÔÉè¶¨µÚÒ»´ÎÆô¶¯µÄÑÓ³ÙÊ±¼ä£¬ÒÔ¼°ºóÐøµÄÖÜÆÚÊ±¼ä
  ...
  // Setup the GAP Peripheral Role Profile
  {
    ...
    uint8_t enableUpdateRequest = DEFAULT_ENABLE_UPDATE_REQUEST;//ÔÊÐíÁ¬½Ó¸üÐÂ²ÎÊý
    ...
    update_scanrsp_data();
    GAPRole_SetParameter(GAPROLE_SCAN_RSP_DATA, sizeof(scanRspData),//ÅäÖÃÉ¨Ãè»ØÓ¦µÄÊý¾Ý
                         scanRspData);

    update_adv_beacon_data();
    GAPRole_SetParameter(GAPROLE_ADVERT_DATA, sizeof(advertData),
                         advertData);//Éè¶¨¹ã²¥µÄÊý¾Ý
...
static void SimpleBLEPeripheral_taskFxn(UArg a0, UArg a1)
{
  ...
  for (;;)
  {
    ...
    if (events & SBP_UART_MONITORING_EVT)//È·¶¨×Ô¶¨ÒåµÄ´®¿Ú¼à²âÊÂ¼þ´¥·¢
    {
     ...
     if (events & SBP_ADV_UPDATE_EVT)//¶¯Ì¬¹ã²¥ÊÂ¼þ
    {
     ...
     if (events & SBP_SCAN_RSP_UPDATE_EVT)//¶¯Ì¬É¨Ãè»ØÓ¦ÊÂ¼þ
    {
